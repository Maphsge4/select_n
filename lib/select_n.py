...
cpu_to_gpu_stream = torch.cuda.Stream(device="cuda")
gpu_to_cpu_stream = torch.cuda.Stream(device="cuda")
forward_stream = torch.cuda.Stream(device="cuda")
...
class ModelShard(nn.Module):
    ...
    def __init__(
        ...
        self._cpu_to_gpu_stream = torch.cuda.Stream(device=self.device)  # 两个stream流，不同shard要用同一个，不要每次初始化！
        self._gpu_to_cpu_stream = torch.cuda.Stream(device=self.device)  # 两个stream流
        # self._cpu_to_gpu_stream = cpu_to_gpu_stream
        # self._gpu_to_cpu_stream = gpu_to_cpu_stream
    ...
    def forward_load(self, non_blocking: bool = True) -> None:
        ...
        nvtx.range_push(f"forward_load{self.index}")
        ...
        for index in range(-1, len(self.model_slices)):
            if index >= 0:
                ...
                with torch.cuda.stream(forward_stream):
                    ...
                    #     print(f"iter {index}", last_inputs[0], flush=True)  # debug  print有隐式同步，因为数据在GPU上
                nvtx.range_pop()
