...
class ShardSyncLayer(torch.autograd.Function):
    ...
    def forward(ctx: Any, inputs: Any, index: int, model_slices: Any, device_list: Any, model_instance: Any) -> Any:  # ctx到底是什么东西？
        ...
        # if drop_index == -1 :
        #     for key_index, value in enumerate(device_list):
        #         if value == 1:
        #             nvtx.range_push(f"forward_load{key_index}")
        #             model_slices[key_index].forward_load()
        #             nvtx.range_pop()
    ...
    def forward(self, *inputs: Any, **_: Any) -> Any:  # 需要改的
        ...
        for index in range(-1, len(self.model_slices)):
            if index >= 0:
                ...
                # if index < len(self.model_slices) - 1:  # maphsge4 delete 0907
                if len(select_list) > 0:
                    select_id = select_list.pop(0)
                    self.model_slices[select_id].forward_load()
                    # torch.cuda.synchronize()  # 草稿版
                print(f"index {index} on-time:", torch.cuda.memory_allocated(device=torch.device("cuda")))  # 显存量
                # print(f"index {index} max:", torch.cuda.max_memory_allocated(device=torch.device("cuda")))  # 显存量
                nvtx.range_pop()
                tmp = time.time() - start
